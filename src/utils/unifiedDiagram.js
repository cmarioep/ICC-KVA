...
export function drawUnifiedDiagram(canvas, data, results) {
  ...
  const {
    ...
    drawGridSymbol, drawResistiveLoadSymbol, drawLoadSymbol, drawBusBar, drawIccLabel,
  ...
  // Asymmetric factor: 1.25 for systems ≤ 600 V, 1.6 above.
  const asymFactor = (kv) => (kv < 0.6 ? 1.25 : 1.6);
  ...
  if (hasGridData) {
    ...
    drawText(`Icc: ${grid.Icc != null ? grid.Icc.toFixed(2) : "--"} kA`, sourceLabelX, gridY + 18, 11, PALETTE.textPrimary);
    drawTextWithKVAccSubscript(`: ${gridKVAcc.toFixed(2)} kVA`, sourceLabelX, gridY + 32, 11, PALETTE.faultCurrent, true);
  }
  ...
  drawIccLabel(`Iasc: ${(mainBusIcc * asymFactor(mainVoltageKV) / 1000).toFixed(2)} kA`, busRightX, mainBusY - 12, 11, PALETTE.faultCurrent, "right");
  ...
  tableros.forEach((tablero, i) => {
    ...
    // kVAcc pushing down into this board's feeder = rest of the network seen at the main
    // bus, i.e. the main bus level minus this board's own upward contribution.
    const restAtMain = mainBusKVAcc - tableroContribToMain;

    const feederLabelTexts = [
      restAtMain.toFixed(2) + " kVA", tableroContribToMain.toFixed(2) + " kVA",
    ...
    drawFlowPairLabels(colX, feederFlow1Y, restAtMain, tableroContribToMain, feederTextStartX);
    if (hasFeeder) {
    ...
    drawIccLabel(`Iasc: ${(tablero.symmetricShortCircuitCurrent * asymFactor(busVoltageKV) / 1000).toFixed(2)} kA`, tBusRightX, tableroBusY - 12, 11, PALETTE.faultCurrent, "right");
    ...
    tablero.loadResults.forEach((load, li) => {
      ...
      if (load.loadType === "resistive") {
        ...
        drawIccLabel(`Iasc: ${(iccTerminal * asymFactor(loadVoltageKV) / 1000).toFixed(2)} kA`, loadX, circuitSymbolY + 52, 11, PALETTE.faultCurrent, "center");
      } else {
